...
const CANONICAL_CONTENT_PATH = path.join(/*turbopackIgnore: true*/ process.cwd(), "data", "portal-content.json");
const DRAFT_CONTENT_PATH = path.join(/*turbopackIgnore: true*/ process.cwd(), "data", "portal-content.draft.json");
const PUBLIC_CONTENT_PATH = path.join(/*turbopackIgnore: true*/ process.cwd(), "public", "data", "portal-content.json");
const BACKUP_CONTENT_DIR = path.join(/*turbopackIgnore: true*/ process.cwd(), "data", ".backups");
...
function loadDraftPortalContent({
  canonicalPath = CANONICAL_CONTENT_PATH,
  draftPath = DRAFT_CONTENT_PATH
} = {}) {
  if (fs.existsSync(/*turbopackIgnore: true*/ draftPath)) {
    return {
      content: loadPortalContent(draftPath),
      draftPath,
      hasDraft: true,
  ...
  return {
    content: loadPortalContent(canonicalPath),
    draftPath,
    hasDraft: false,
...
function writePortalContent(content, filePath = CANONICAL_CONTENT_PATH) {
  fs.writeFileSync(/*turbopackIgnore: true*/ filePath, formatPortalContent(content), "utf8");
}

function backupPortalContent({
  canonicalPath = CANONICAL_CONTENT_PATH,
  backupDir = BACKUP_CONTENT_DIR
} = {}) {
  ...
  const backupPath = path.join(backupDir, `portal-content-${timestamp}.json`);
  fs.copyFileSync(/*turbopackIgnore: true*/ canonicalPath, backupPath);
...
function syncPortalContent({
  canonicalPath = CANONICAL_CONTENT_PATH,
  publicPath = PUBLIC_CONTENT_PATH
} = {}) {
  const content = loadPortalContent(canonicalPath);
  validatePortalContent(content);
  fs.mkdirSync(path.dirname(publicPath), { recursive: true });
  fs.copyFileSync(/*turbopackIgnore: true*/ canonicalPath, publicPath);
}

function savePortalContent(content, {
  canonicalPath = CANONICAL_CONTENT_PATH,
  publicPath = PUBLIC_CONTENT_PATH,
  backupDir = BACKUP_CONTENT_DIR
} = {}) {
  validatePortalContent(content);
  const backupPath = backupPortalContent({ canonicalPath, backupDir });
  const previousContent = fs.existsSync(/*turbopackIgnore: true*/ canonicalPath)
    ? loadPortalContent(canonicalPath)
    : null;
  const changed = JSON.stringify(previousContent) !== JSON.stringify(content);

  if (changed) {
    fs.mkdirSync(path.dirname(canonicalPath), { recursive: true });
    writePortalContent(content, canonicalPath);
    syncPortalContent({ canonicalPath, publicPath });
  }
...
function savePortalDraft(content, {
  draftPath = DRAFT_CONTENT_PATH
} = {}) {
  validatePortalContent(content);
  const previousContent = fs.existsSync(/*turbopackIgnore: true*/ draftPath)
    ? loadPortalContent(draftPath)
    : null;
  const changed = JSON.stringify(previousContent) !== JSON.stringify(content);

  if (changed) {
    fs.mkdirSync(path.dirname(draftPath), { recursive: true });
    writePortalContent(content, draftPath);
  }
...
function publishPortalContent(content, {
  canonicalPath = CANONICAL_CONTENT_PATH,
  publicPath = PUBLIC_CONTENT_PATH,
  draftPath = DRAFT_CONTENT_PATH,
  backupDir = BACKUP_CONTENT_DIR
  ...
  if (fs.existsSync(/*turbopackIgnore: true*/ draftPath)) {
    fs.rmSync(/*turbopackIgnore: true*/ draftPath);
  }
...
function validatePortalContent(content) {
  ...
  requireObject(content.sections, "sections", errors);
  requireArray(content.trainingResources, "trainingResources", errors);
  ...
  validateSections(content.sections || {}, errors);
  validateCourses(content.trainingResources || [], errors, "trainingResources");
...
function validateCourses(courses, errors, pathRoot = "courses") {
  validateUniqueIds(courses, pathRoot, errors);

  courses.forEach((course, index) => {
    const pathLabel = `${pathRoot}[${index}]`;
...
function validateSections(sections, errors) {
  requireObject(sections.trainingResources, "sections.trainingResources", errors);
  requireString(sections.trainingResources?.eyebrow, "sections.trainingResources.eyebrow", errors);
  requireString(sections.trainingResources?.title, "sections.trainingResources.title", errors);
}
